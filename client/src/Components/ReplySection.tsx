import type { replySection, Reply as ReplyType } from "../Types/Types"
...
function ReplySection({ user, comment, commentReply, setReply, reply, index }: replySection) {
    const [replyContent, setReplyContent] = useState<string>(commentReply.content);
    ...
    const editComment = async (replies: ReplyType[]) => {
        try {
            const update = await fetch(import.meta.env.VITE_BACKEND_URL, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
    ...
    const handleIncrease = (): void => {
        const newReplies = comment.replies.map((reply, i) =>
            i === index ? { ...reply, score: reply.score + 1 } : reply
        );

        updateComment(comment._id, comment.content, comment.createdAt, comment.score, comment.user, newReplies);
    }
    ...
    const handleEditButton = (): void => {
        setEdit(!edit);
        setReplyContent(commentReply.content)
    }

    const handleEdit = (): void => {
        const newReplies = comment.replies.map((reply, i) =>
            i === index ? { ...reply, content: replyContent } : reply
        );

        editComment(newReplies);
    }

    return (
        <>
            {remove && <Delete id={index} comment={comment} setRemove={() => setRemove(prev => !prev)} />}
                ...
                <div className="comment-body">
                    <div className="comment-top">
                        <div className="comment-info">
                            ...
                            <p className="post-time">{commentReply.createdAt}</p>
                        </div>
                    ...
                    {edit ?
                        <textarea className="comment-content" value={replyContent} name="edit-reply"
                            onChange={(e) => setReplyContent(e.target.value)}></textarea> :
                        <div className="comment-content">
                            <span className="tag">{`@${commentReply.replyingTo} `}</span>
                            {replyContent}
                        </div>}
